...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("tracks in-flight background work across turn boundaries", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      const pendingBackgroundWork = Effect.gen(function* () {
        const sessions = yield* adapter.listSessions();
        return sessions[0]?.pendingBackgroundWork;
      });
      ...
      assert.equal(yield* pendingBackgroundWork, undefined);
      ...
      assert.deepEqual(yield* pendingBackgroundWork, {
        count: 2,
        kinds: ["local_bash", "monitor"],
      });

      // A terminal `task_updated` retires the entry on its own — some tasks
      // never emit a notification.
      harness.query.emit({
        type: "system",
      ...
      assert.deepEqual(yield* pendingBackgroundWork, { count: 1, kinds: ["monitor"] });

      // A non-terminal patch must not retire anything: this is exactly the
      // state a monitor sits in while it waits to fire.
      harness.query.emit({
        type: "system",
        subtype: "task_updated",
        task_id: "monitor-1",
  ...
  it.effect("reconciles in-flight background work from the roster snapshot", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const drain = Effect.gen(function* () {
      ...
      harness.query.emit({
        ...
        description: "Missed its notification",
        session_id: "session",
        uuid: "ts-stale",
      } as unknown as SDKMessage);
      yield* drain;

      assert.deepEqual(yield* pendingBackgroundWork, { count: 1, kinds: ["local_agent"] });

      // Roster is authoritative: it prunes the stale entry, keeps the live
      // one, and drops entries that already reached a terminal status.
      harness.query.emit({
        type: "system",
        subtype: "background_tasks_changed",
        tasks: [
          { task_id: "monitor-1", task_type: "monitor", description: "Watch CI" },
          { id: "shell-1", type: "local_bash", status: "running", description: "pnpm test" },
          { id: "shell-2", type: "local_bash", status: "completed", description: "done" },
        ],
        session_id: "session",
        uuid: "roster-1",
      } as unknown as SDKMessage);
      ...
      // An unparseable roster leaves the tracked set alone rather than
      // wiping protection off a session that is still working.
      harness.query.emit({
        type: "system",
        subtype: "background_tasks_changed",
        session_id: "session",

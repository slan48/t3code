...
interface MessagesTimelineProps {
  ...
  workspaceRoot: string | undefined;
  /**
   * What this thread calls its proposed plan.
   *
   * Defaults to the coding wording so an existing caller renders exactly what
   * it renders today; a Navigator thread passes the Execution Proposal wording
  ...
  return (
    <>
      <div className="relative min-w-0 px-1 py-0.5">
        ...
        />
        {/*
          Touch devices never fire hover, so the copy action would stay stuck at
          opacity-0 on mobile web. Coarse pointers pin the meta row visible; fine
          pointers keep the hover-reveal behaviour.
        */}
...
function WorkingTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "working" }> }) {
  const workingTokens = formatWorkingTokens(row.contextWindow);
  return (
    <div className="py-0.5 pl-1.5">
      <div className="flex items-center gap-2 pt-1 text-[11px] text-muted-foreground/70 tabular-nums">
        ...
        {workingTokens && <span className="text-muted-foreground/55">· {workingTokens}</span>}
      </div>
    </div>
  );
}
